...
async function presentView(v){
 ...
 let stage='';if(type.startsWith('image/'))stage=`<div class="present-stage image"><img src="${url}" alt="${esc(f.name)}"></div>`;else if(type==='application/pdf'&&!native)stage=`<div class="present-stage pdf"><iframe title="${esc(f.name)}" src="${url}"></iframe></div>`;else stage=`<div class="present-stage native-pdf"><span>${icon('file')}</span><b>PDF 凭证</b><p>${esc(f.name)}</p>${btn('${window.TuhuNative?.platform?.()==='ios'?'打开 / 分享 PDF':'用手机 PDF 阅读器打开'}','native-file',{id:f.id},'button wallet-primary')}</div>`;
