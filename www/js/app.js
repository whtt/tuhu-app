/* 兔狐 v55 — field-use polish on top of the v54 wallet baseline. */
(()=>{
'use strict';
...
const typeLabel={'scene-more':'更多',scene:'',assistant:'旅途助手',execution:'执行',map:'地图',photo:'拍照参考',capture:'拍摄',guide:'导览',english:'英语',notes:'备忘',wallet:'票夹',booking:'订单资料',present:'出示凭证',file:'本机文件',food:'吃喝',hotel:'住宿',zone:'园区',dates:'选择日期',more:'更多',tasks:'待办',search:'搜索',settings:'设置',backup:'数据备份',lightbox:'拍照参考',field:'现场步骤',overview:'当天行程',phrase:'英语'};
const typeIcon={assistant:'sparkle',execution:'flag',map:'route',photo:'camera',capture:'settings',guide:'guide',english:'english',notes:'note',wallet:'wallet',food:'food',hotel:'hotel',zone:'pin',tasks:'check',more:'more',search:'search',settings:'settings',backup:'file',file:'file',booking:'ticket',present:'ticket',field:'flag'};
...
async function renderView(){
 ...
 try{
  switch(v.type){
   ...
   case'backup':html=backupView();break;
...
function englishRows(v){
 ...
 const source=q?cats.flatMap(cat=>(D.ENGLISH_SETS[cat]||[]).map(row=>({cat,row}))):(D.ENGLISH_SETS[v.cat]||[]).map(row=>({cat:v.cat,row}));
 const seen=new Set();
...
async function presentView(v){
 ...
 v.index=clamp(Number(v.index)||0,0,files.length-1);const f=files[v.index],type=fileType(f),blob=new Blob([f.data],{type}),url=objectURL(blob),native=window.TuhuNative?.isNative?.();v.currentFileId=f.id;
 let stage='';if(type.startsWith('image/'))stage=`<div class="present-stage image"><img src="${url}" alt="${esc(f.name)}"></div>`;else if(type==='application/pdf'&&!native)stage=`<div class="present-stage pdf"><iframe title="${esc(f.name)}" src="${url}"></iframe></div>`;else stage=`<div class="present-stage native-pdf"><span>${icon('file')}</span><b>PDF 凭证</b><p>${esc(f.name)}</p>${btn('用手机 PDF 阅读器打开','native-file',{id:f.id},'button wallet-primary')}</div>`;
...
async function fileView(v){
 ...
 else preview=`<div class="empty">PDF · ${sizeText(f.size||blob.size)}<br>用手机上的 PDF 阅读器打开</div>`;
...
async function exportBackup(b){if(b)b.disabled=true;try{const files=await filesAll(),values={};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(legacyKey(k))values[k]=storage.get(k)}const records=[];for(const f of files)records.push({...f,data:await base64(f.data)});const data={format:'tuhu-backup',version:1,app:'v55',created:new Date().toISOString(),storage:values,files:records};await saveBlob('tuhu-backup-'+new Date().toISOString().slice(0,10)+'.json',new Blob([JSON.stringify(data)],{type:'application/json'}));toast('备份已交给系统保存')}catch(e){toast('导出失败：'+(e.message||'本机存储不可用'))}finally{if(b)b.disabled=false}}
...
function moreView(v){return `<div class="product-grid">${[['assistant','旅途助手'],['tasks','待办'],['search','搜索'],['notes','当天备忘'],['wallet','票夹'],['food','吃喝'],['hotel','住宿'],['photo','拍照'],['settings','设置'],['backup','备份']].map(([t,label])=>t==='assistant'?tile(t,getSelectedKey(),viewDay(v),label):tile(t,null,viewDay(v),label)).join('')}</div><div class="version-line">兔狐 · v55</div>`}
function settingsView(){
 const feel=storage.get('tuhu-motion-feel')||'light';
 return `<div class="surface-card"><div class="setting-row"><span>版本</span><span>兔狐 v55</span></div><div class="setting-row"><span>行程</span><span>2026.10.02 — 10.07</span></div><div class="setting-row"><span>离线英语</span><span>${Object.keys(A).length} 条本地录音</span></div><div class="setting-row"><span>旅途助手</span><span>iPhone 内置知识库 / Android 可选 GGUF</span></div></div><h3 class="section-label">离线 AI</h3>${btn(icon('sparkle')+' 打开旅途助手','feature',{type:'assistant',key:getSelectedKey(),day:state.day},'button secondary full')}<p class="hint">Android 可导入一次 GGUF 模型，之后完全离线推理；没有模型也不影响行程、英语、票夹等功能。</p><h3 class="section-label">滑动手感</h3><div class="feel-options">${[['light','轻快'],['balanced','标准']].map(([id,name])=>btn(name,'motion-feel',{feel:id},'chip'+(feel===id?' active':''))).join('')}</div><h3 class="section-label">声音</h3><div class="button-row equal">${btn(icon('sound')+' 试听一句','say',{text:'Hello, excuse me.'},'button secondary')}${btn('停止','stop-audio',{},'button outline')}</div><p class="hint">常用英语直接播放随 App 打包的离线录音，不需要先检测系统语音。无声时只需检查媒体音量和蓝牙输出。</p><h3 class="section-label">资料</h3>${btn('备份 / 恢复','feature',{type:'backup'},'button secondary full')}<p class="hint">资料只保存在本机；卸载前请导出备份。</p><div class="version-line">TUHU · 55.0.0</div>`;
}
...
renderHome();
if('serviceWorker'in navigator&&location.protocol!=='file:'&&!window.__TUHU_SINGLE__&&!window.TuhuNative?.isNative?.()){navigator.serviceWorker.register('./sw.js').catch(()=>{})}
})();