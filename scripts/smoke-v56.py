...
ROOT=Path('/mnt/data/v56_work/tuhu_v56_app/www')
OUT=Path('/mnt/data/v56_work/tuhu_v56_app/dist')
...
def make_page(browser):
    ...
    p.wait_for_function("window.Tuhu && window.Tuhu.version === 'v56'",timeout=30000)
...
with sync_playwright() as pw:
    ...
    p.screenshot(path=str(OUT/'tuhu_v56_home_day03.png'),full_page=False)
    ...
    p.evaluate("(()=>{const t=document.querySelector('#toast');if(t)t.style.display='none'})()")
    p.screenshot(path=str(OUT/'tuhu_v56_scene_merlion.png'),full_page=False)
    ...
    p.evaluate("(()=>{const t=document.querySelector('#toast');if(t)t.style.display='none'})()")
    p.screenshot(path=str(OUT/'tuhu_v56_english.png'),full_page=False)
    ...
    p.evaluate("(()=>{const t=document.querySelector('#toast');if(t)t.style.display='none'})()")
    p.screenshot(path=str(OUT/'tuhu_v56_wallet.png'),full_page=False)
...
(OUT/'v56-browser-smoke.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),'utf-8')
